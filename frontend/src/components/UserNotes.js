...
function UserNotes() {
  ...
  let handleSubmit  = async (e) => {
      ...
      let res = await fetch('http://localhost:8000/api/notes/',{
       method:"GET",
...
const handleClickPost = (noteId) =>{
  router.push(`post/${noteId}`)
    ...
    <div className='grid mt-24 mb-10 ml-20 place-content-center'>
      {auth ? <h1 className='text-white text-[25px] font-bold leading-[70px] tracking-wide font-outfit'>welcome to ur account {username}</h1> : <h1>you not authorized to see this userNotes </h1>}
      {notes.map((note) => {
        return(
          <motion.div
